...
De dynamische route `/wat-we-doen/[slug]` rendert gepubliceerde documenten uit de Payload-collectie **Diensten**. De zes goedgekeurde diensten hebben daarnaast gecontroleerde codefallbacks, zodat de dienstpagina's lokaal beoordeeld kunnen worden voordat de CMS-content is ingevoerd.
...
Titel, slug, groep en korte samenvatting zijn verplicht. Alle bezoekerssecties zijn conditioneel: een lege sectie wordt niet gerenderd. Certificeringen en standaarden verschijnen uitsluitend wanneer `Inhoudelijk gevalideerd voor publicatie` aanstaat. Gepubliceerde CMS-inhoud heeft voorrang op de codefallback; ontbrekende optionele inhoud kan per goedgekeurde dienst veilig uit de fallback worden aangevuld.
