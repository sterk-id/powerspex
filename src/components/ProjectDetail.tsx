...
export function ProjectDetail({ project }: { project: ProjectDetailData }) {
  ...
  return <main id="main-content">
    ...
    {project.contentSections.map((section) => <section className={`detail-content-section ${section.theme}`} key={`${section.eyebrow}-${section.title}`}><div className={`container detail-content-grid ${section.layout}`}><div className="detail-content-copy">{section.eyebrow && <span className={`eyebrow ${section.theme === 'dark' ? 'light' : ''}`}>{section.eyebrow}</span>}<h2>{section.title}<span className="dot">.</span></h2><p>{section.body}</p></div>{section.image?.src && <div className="detail-content-image"><ProjectImage image={section.image} /></div>}</div></section>)}
