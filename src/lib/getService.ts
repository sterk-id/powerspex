...
import { hardwareEngineering, processAutomation, production, projectEngineering, softwareEngineering, type ServiceDetailData } from '@/data/services'
...
function completeService(data: ServiceDetailData): ServiceDetailData {
  const fallback = [projectEngineering, processAutomation, softwareEngineering, hardwareEngineering, production].find((item) => item.slug === data.slug)
...
async function loadService(slug: string): Promise<ServiceDetailData | null> {
  ...
  return [projectEngineering, processAutomation, softwareEngineering, hardwareEngineering, production].find((service) => service.slug === slug) ?? null
}
